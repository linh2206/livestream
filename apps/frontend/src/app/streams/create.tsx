...
export default function CreateStreamPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const loadingToastId = showLoading('Creating Stream', 'Setting up your stream...');
    ...
    try {
      ...
      if (err.response?.status === 401) {
        showError('Authentication Required', 'Please log in again to create streams.');
        router.push('/login');
      } else if (err.response?.status === 404) {
        showError('User Not Found', 'Your account could not be found. Please log in again.');
        router.push('/login');
      } else if (err.response?.status === 403) {
        showError('Access Denied', 'You do not have permission to create streams.');
      } else {
      ...
      <div className="flex">
        ...
        <main className="flex-1 p-6">
          <div className="max-w-4xl mx-auto">
            ...
            <Card className="mt-8">
              <h3 className="text-lg font-semibold text-white mb-4">Streaming Instructions</h3>
              <div className="space-y-4 text-gray-300">
                <div>
                  <h4 className="font-medium text-white mb-2">1. Get your Stream Key</h4>
                  <p className="text-sm">After creating the stream, you'll receive a unique stream key.</p>
                </div>
                <div>
                  <h4 className="font-medium text-white mb-2">2. Configure your streaming software</h4>
                  <p className="text-sm">Use OBS Studio, XSplit, or any RTMP-compatible software.</p>
                </div>
                <div>
