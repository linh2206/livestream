...
export default function StreamDetailPage() {
  ...
  const { user } = useAuth();
  const { socket, joinStreamChat, leaveStreamChat, joinStream, leaveStream } = useSocketContext();
  ...
  }, [stream?._id, user?._id, socket, joinStreamChat, leaveStreamChat, joinStream, leaveStream]);
  ...
  const handleLike = async () => {
    ...
    try {
      ...
      setStream(prev => prev ? { ...prev, likeCount: result.stream.likeCount } : null);
    } catch (error) {
      console.error('Error liking stream:', error);
      // Rollback on error
      setIsLiked(previousIsLiked);
      setStream(prev => prev ? { ...prev, likeCount: previousLikeCount } : null);
    }
  };
