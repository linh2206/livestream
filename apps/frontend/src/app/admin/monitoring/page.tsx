        ...
        {stats && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            <div className="bg-gray-800 rounded-lg p-4">
              <h3 className="text-xs font-medium text-gray-400 mb-1">CPU Usage</h3>
              <p className="text-2xl font-bold">{stats.system.cpu.usage.toFixed(1)}%</p>
            </div>
            <div className="bg-gray-800 rounded-lg p-4">
              <h3 className="text-xs font-medium text-gray-400 mb-1">Memory Usage</h3>
              <p className="text-2xl font-bold">{stats.system.memory.percentage.toFixed(1)}%</p>
            </div>
            <div className="bg-gray-800 rounded-lg p-4">
              <h3 className="text-xs font-medium text-gray-400 mb-1">Active Users</h3>
              <p className="text-2xl font-bold">{stats.application.activeUsers}</p>
            </div>
            <div className="bg-gray-800 rounded-lg p-4">
              <h3 className="text-xs font-medium text-gray-400 mb-1">Active Streams</h3>
              <p className="text-2xl font-bold">{stats.application.activeStreams}</p>
            </div>
          </div>
        )}
              ...
              <tbody>
                {userActivity.map((user) => (
                  <tr key={user.userId} className="border-t border-gray-700">
                    ...
                    <td className="px-4 py-2">{new Date(user.lastLogin).toLocaleString()}</td>
            ...
            <table className="w-full">
              ...
              <tbody>
                {streamAnalytics.map((stream) => (
                  <tr key={stream.streamId} className="border-t border-gray-700">
                    <td className="px-4 py-2">{stream.title}</td>
                    <td className="px-4 py-2">{stream.streamer}</td>
                    <td className="px-4 py-2">{Math.floor(stream.duration / 60)}m</td>
                    ...
                    <td className="px-4 py-2">
                      <span className={`px-2 py-1 rounded text-xs ${
                        stream.status === 'live' ? 'bg-red-600' : 'bg-gray-600'
                      }`}>
                        {stream.status}
                      </span>
                    </td>
