...
export class HlsController {
  ...
  @Get('public/:streamKey')
  async servePublicHlsPlaylist(
    @Param('streamKey') streamKey: string,
    @Res() res: Response
  ) {
    try {
      if (process.env.NODE_ENV !== 'production') {
        // eslint-disable-next-line no-console
        console.log(
          `🎬 [HLS Public] Playlist request for stream: ${streamKey}`
        );
      }
      ...
      if (!existsSync(playlistPath)) {
        if (process.env.NODE_ENV !== 'production') {
          // eslint-disable-next-line no-console
          console.log(`❌ [HLS Public] File not found: ${playlistPath}`);
        }
      ...
      res.setHeader(
        ...
        'Content-Type, Range, Authorization, Cache-Control, If-Range, If-Modified-Since, If-None-Match, Pragma'
      );
      res.setHeader('Accept-Ranges', 'bytes');
      ...
      const modifiedLines = lines.map(line => {
        ...
        if (line.endsWith('.ts') && !line.startsWith('/api/v1/hls/')) {
          return `/api/v1/hls/public/${streamKey}/${line}`;
        }
        return line;
      });
      ...
      if (process.env.NODE_ENV !== 'production') {
        // eslint-disable-next-line no-console
        console.log(
          `📝 [HLS Public] Original: ${originalContent.replace(/\n/g, '\\n')}`
        );
        // eslint-disable-next-line no-console
        console.log(
          `📝 [HLS Public] Modified: ${playlistContent.replace(/\n/g, '\\n')}`
        );
      }
      ...
      // eslint-disable-next-line no-console
      console.error(
        `❌ [HLS Public Controller] Error serving playlist for ${streamKey}:`,
        error
      );
      throw new StreamOfflineException('Stream is currently offline');
    }
  ...
  @Get('public/:streamKey/:filename')
  async servePublicHlsSegment(
    @Param('streamKey') streamKey: string,
    @Param('filename') filename: string,
    @Res() res: Response
  ) {
    try {
      // eslint-disable-next-line no-console
      console.log(
        `🎥 [HLS Public Controller] Serving segment: ${filename} for stream: ${streamKey}`
      );
      ...
      if (!existsSync(segmentPath)) {
        // eslint-disable-next-line no-console
        console.log(
          `❌ [HLS Public Controller] Segment not found: ${segmentPath}`
        );
        throw new StreamNotFoundException('Segment not found');
      }
      ...
      // eslint-disable-next-line no-console
      console.error(
        `❌ [HLS Public Controller] Error serving segment ${filename} for ${streamKey}:`,
        error
      );
      throw new StreamNotFoundException('Segment not found');
    }
